...
export default {
  ...
  methods: {
    ...
    addshopcart(){
      //1.发请求，将信息添加到服务器数据库购物车
      //2.服务器存储成功，进行路由跳转
      //3.失败，给用户提示
      //4.在路由跳转时将详情信息传给下一层级路由组件
      //trycatch也就是捕捉AddOrUpdateShopCart的Promise对象返回值，如果失败就提示错误
      try {
        //发请求
         this.$store.dispatch("AddOrUpdateShopCart",{skuid:this.$route.params.skuid,skuNum:this.skuNum})
         //本地存储，因为不能存对象，所以转化为json字符串
        //  console.log(this.skuInfo);
        // 这个功能没有做完善点击属性之后只修改元素属性的高亮，并没有改变skuInfo中的属性
